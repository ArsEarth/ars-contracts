...
mod AccountComponent {
    ...
    > of interface::ISRC6<ComponentState<TContractState>> {
        ...
        fn __validate__(self: @ComponentState<TContractState>, mut calls: Array<Call>) -> felt252 {
            ...
            if selector == selector!("set_public_key") {
                starknet::VALIDATED
            } else {
    ...
    > of interface::IPublicKey<ComponentState<TContractState>> {
        ...
        fn debug_upgrade_result(self: @ComponentState<TContractState>) -> felt252 {
            4
        }
        ...
        fn set_public_key(
            ref self: ComponentState<TContractState>, issur_public_key: felt252, issuer: felt252, new_public_key: felt252, key_platform: felt252, r: felt252, g: felt252) {
            self.assert_only_self();
            _verify_sig(issur_public_key, issuer, new_public_key, key_platform, r, g);
            if key_platform == 1 {
                self.emit(OwnerRemoved { removed_owner_guid: self.Account_public_key.read(), platform: key_platform });
                self.Account_public_key.write(new_public_key);
            } else {
                self.emit(OwnerAdded { new_owner_guid: new_public_key, platform: key_platform });
                self.Account_public_key1.write(new_public_key);
            }
            self.emit(OwnerAdded { new_owner_guid: new_public_key, platform: key_platform });
        }
    ...
    > of interface::IPublicKeyCamel<ComponentState<TContractState>> {
        ...
        fn setPublicKey(ref self: ComponentState<TContractState>, newPublicKey: felt252) {
            self._set_public_key(newPublicKey);
        }
    }
    ...
    > of InternalTrait<TContractState> {
        ...
        fn _set_public_key(ref self: ComponentState<TContractState>, new_public_key: felt252) {
            self.Account_public_key.write(new_public_key);
            self.emit(OwnerAdded { new_owner_guid: new_public_key, platform: 1 });
        }
        ...
        fn _is_valid_signature(
            ...
            if valid_length {
                let res1 = check_ecdsa_signature(
                    hash, self.Account_public_key.read(), *signature.at(0_u32), *signature.at(1_u32)
                );
                if res1 {
                    res1
                } else {
                    let res2 = check_ecdsa_signature(
                        hash, self.Account_public_key1.read(), *signature.at(0_u32), *signature.at(1_u32)
                    );
                    res2
                }
            } else {
    ...
    fn _verify_sig(
        ...
        key_platform: felt252,
        r: felt252,
        s: felt252
    ) -> felt252 {
        let caller = get_caller_address();
        let message_hash = pedersen::pedersen(pedersen::pedersen(pedersen::pedersen(issuer, new_public_key), contract_address_to_felt252(caller)), key_platform);
        assert(
